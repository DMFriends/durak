...
JAVAFX_VERSION="${JAVAFX_VERSION:-26}"
PACKAGE_TYPE="${PACKAGE_TYPE:-}"
VERSION="${APP_VERSION:-1.0}"
JAVA_RELEASE="${JAVA_RELEASE:-26}"
...
javac "${JAVAC_ARGS[@]}" $(find src/application -name '*.java' | sort)

cp -R src/resources "$BUILD_DIR/classes/resources"

jar \
  --create \
  --file "$INPUT_DIR/$APP_JAR" \
  --main-class "$MAIN_CLASS" \
  -C "$BUILD_DIR/classes" .

ICON_ARGS=()
if [[ "$(uname -s)" == "Linux" ]]; then
  ICON_ARGS+=(--icon src/resources/durak.png)
elif [[ "$(uname -s)" == "Darwin" ]]; then
  ICONSET="$BUILD_DIR/Durak.iconset"
  ICNS="$BUILD_DIR/Durak.icns"
  rm -rf "$ICONSET"
  mkdir -p "$ICONSET"
  sips -z 16 16 src/resources/durak.png --out "$ICONSET/icon_16x16.png" >/dev/null
  sips -z 32 32 src/resources/durak.png --out "$ICONSET/icon_16x16@2x.png" >/dev/null
  sips -z 32 32 src/resources/durak.png --out "$ICONSET/icon_32x32.png" >/dev/null
  sips -z 64 64 src/resources/durak.png --out "$ICONSET/icon_32x32@2x.png" >/dev/null
  sips -z 128 128 src/resources/durak.png --out "$ICONSET/icon_128x128.png" >/dev/null
  sips -z 256 256 src/resources/durak.png --out "$ICONSET/icon_128x128@2x.png" >/dev/null
  sips -z 256 256 src/resources/durak.png --out "$ICONSET/icon_256x256.png" >/dev/null
  sips -z 512 512 src/resources/durak.png --out "$ICONSET/icon_256x256@2x.png" >/dev/null
  sips -z 512 512 src/resources/durak.png --out "$ICONSET/icon_512x512.png" >/dev/null
  sips -z 1024 1024 src/resources/durak.png --out "$ICONSET/icon_512x512@2x.png" >/dev/null
...
if [[ "$ENABLE_PREVIEW" == "true" ]]; then
  JPACKAGE_ARGS+=(--java-options --enable-preview)
fi

if [[ "$PACKAGE_TYPE" == "deb" ]]; then
  JPACKAGE_ARGS+=(--linux-package-name durak --linux-app-release 1 --linux-shortcut)
fi
